...
| 26 | **🔴 异常分析宪法：必须输出完整因果链架构图** | **每个 bug/异常的处理流程，无例外：** ① 先输出**完整因果链架构图**（从上游到下游的 ASCII 流程图，标注每一跳的：组件名 → 具体错误 → 传播路径 → 最终用户影响）② 再做三层根因分析：**触发器**（什么外部事件引爆）→ **放大器**（什么架构缺陷让影响扩散）→ **掩护者**（什么缺失让问题被隐藏到用户发现）③ 还原时间线（精确到分钟级）④ 回答"为什么以前没发生"（条件组合分析）⑤ 将案例沉淀到 `ontology/docs/cases/` 并提炼 governance 不变式。**因果链架构图是强制输出，不是可选项** — 系统层级越多，越需要可视化爆炸链让每次事故成为后续反思的教科书。修代码之前先画图，画不清因果链说明还没理解问题。（2026-04-10 升级为宪法级：从"原则"到"无例外强制"） |
