...
| 26 | **🆕 异常必须深挖爆炸链，喂养本体工程** | 每个 bug/异常不是修完就结束，必须追溯完整因果链：**触发器**（什么外部事件引爆）→ **放大器**（什么架构缺陷让影响扩散）→ **掩护者**（什么缺失让问题被隐藏到用户发现）。三层根因分析后，将案例沉淀到 `ontology/docs/cases/` 作为活证据。每个案例自然产出：新的 governance 不变式、新的检查维度、新的设计原则。系统稳定性不是靠加功能，而是靠每次事故把防线织得更密。（2026-04-10教训：Dream MapReduce 配额耗尽→HN 静默推送垃圾，三层根因缺一不可） |
