...
> 每次新会话开始时自动读取。当前版本：v37.8.3 / 0.37.8.3（2026-04-13）
...
| `ontology/` | **V36.1新增→V37.8升级** Ontology 独立子项目 — Tool Engine（81条声明式规则+推理引擎+**classify_tool_call语义分类**）+ **Governance Ontology v3.9**（**35不变式**+139可执行检查+**9元规则**+**MR-6 critical-invariants-need-depth 从警告升级为 hard-enforcing via INV-LAYER-001**+MR-7 治理自观察+MR-8 copy-paste-is-a-bug-class+MR-9 state-writes-go-through-helper+**INV-GOV-001 summary 不吞 error**+**INV-DREAM-001/002 + INV-CACHE-002 Dream Map 预算+缓存契约（V37.8 补齐运行时层）**+**INV-PA-001/002 告警污染+SOUL.md 规则 10 双防线**+**INV-LAYER-001 治理深度自我强制**+验证深度三层模型+Phase 0元规则自主发现+MRD-LAYER-001/MRD-NOTIFY-002 检测器重写）+ governance_checker.py 执行引擎（V37.2+ silent error 修复：error 状态计入 failed_invs, V37.8 `_discover_silent_channels` 双层检测 source+activity）+ 宪法6条 + 语义查询PoC + **Phase 2 shadow模式**(off→shadow→on) + 立场文章(EN+ZH) |
...
| `jobs/finance_news/run_finance_news.sh` | **V37.8.2新增** 全球财经/政策新闻每日汇总（双通道：8 RSS 直连 + 21 X/Twitter Syndication API，72h 窗口覆盖周末，LLM 结构化分析+价值评级+国内外对比+投资建议+风险提示，cron 07:30） |
...
> **每次使用 Claude Code 必须使用最强模型（当前：Claude Opus 4.6）。**
>
> 禁止为了"快一点"切换到较弱模型（Sonnet/Haiku），禁止开启 fast mode 牺牲推理深度，禁止因为 token 用量或响应速度而降级。
>
> 这个项目的核心竞争力是**思考深度和工程质量**，不是速度。用弱模型省下的 10 分钟，会在后续的 bug 修复、连锁故障、返工中付出 10 倍代价。
>
> **951 个测试、35 个治理不变式、10 条元规则——这个系统的复杂度要求最强的推理能力来驾驭，没有妥协空间。**
>
> **执行机制（2026-04-14 强制）**：每次新 session 开工的**第一条回复**必须包含一行模型声明，格式固定为：
> `🤖 本次 session 模型：Claude Opus 4.6 (claude-opus-4-6) | 最强级别，无降级 | fast mode 状态见 /fast 命令`
> 声明放在开工报告之前，独占一行，前面不加任何寒暄。如果模型 ID 不是 `claude-opus-4-6`（例如被错配到 Sonnet/Haiku），**立即停止工作并告知用户**。见原则 #30。
...
| 30 | **🔴🔴🔴 开工必须主动声明模型（第 0 号宪法执行机制）** | 每次新 session 开工的**第一条回复必须以模型声明开头**，独占一行，格式固定：`🤖 本次 session 模型：Claude Opus 4.6 (claude-opus-4-6) \| 最强级别，无降级 \| fast mode 状态见 /fast 命令`。**用户无需询问也必须声明**，这是第 0 号宪法的自动执行机制，不是可选项。如果运行时检测到模型 ID 不是 `claude-opus-4-6`（即被错配到 Sonnet 4.6 / Haiku 4.5 或其他），**立即停止工作，把错配告知用户，等待处理后再继续**。禁止"先干活再说"的侥幸思维——弱模型写的代码会在后续造成连锁故障。声明位置：在任何寒暄、分析、工具调用之前。（2026-04-14教训：用户必须手动追问"是否最强模型"才能确认，声明应主动前置而非被动回答） |
