...
> 每次新会话开始时自动读取。当前版本：v37.9.12 / 0.37.9.12（2026-04-22）
...
| `ontology/policy_ontology.yaml` | **V37.9.9新增（骨架）→V37.9.12 P1 wiring 依据** Phase 4 Layer 2 策略声明 — 10 条 policy 按 static/temporal/contextual 三类分：max-tools-per-agent / alert-context-isolation / quiet-hours-00-07 / dream-map-budget / multimodal-routing / fallback-chain-capability 等 + 策略冲突检测 stub + 元规则 MR-7/8/16 引用。**V37.9.12 给 3 条 hard_limit policy 加 `limit` 字段**（max-tools-per-agent=12, max-tool-calls-per-task=2, max-request-body-size=200000），作为机器可读阈值供 `evaluate_policy()` 返回。|
