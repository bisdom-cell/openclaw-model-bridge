...
> 每次新会话开始时自动读取。当前版本：v37.0 / 0.37.0（2026-04-08）
...
| `kb_harvest_chat.py` | **V37新增** 对话精华提炼器（从 proxy 捕获的每日对话中 LLM 提取决策/偏好/洞察/结论，写入 KB notes，每日 06:00 cron） |
...
| 9 | **🔴 收工必须刷新全部项目文档 + 健康检查** | "结束今天的工作"时，以下步骤**全部完成才算收工**：① **全量扫描并刷新所有相关文档**：逐一 check 并更新——`status.json`（recent_changes/priorities/session_context/focus）、`CLAUDE.md`（版本号/文件表/变更历史/待办状态/常用命令）、`docs/config.md`（如有配置变更）、`jobs_registry.yaml`（如有任务变更）、`README.md`（如有架构变更）、`SOUL.md`（如有 PA 行为变更）、`docs/ontology/`（如有 ontology 内容变更）→ **全部提交推送** ② `security_score.py --update` 写入安全评分 ③ 提醒用户在 Mac Mini 上运行 `bash preflight_check.sh --full` + `bash job_smoke_test.sh` ④ **不刷新就收工 = 下次开工浪费时间重新理解上下文**（2026-04-06教训：session 间状态不同步导致重复工作） |
