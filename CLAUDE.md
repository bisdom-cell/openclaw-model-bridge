...
> 每次新会话开始时自动读取。当前版本：v37.9.16 / 0.37.9.16（2026-04-24）
...
| `test_kb_deep_dive.py` | **V37.9.16新增** kb_deep_dive 单测（55 个用例 / 10 类）：TestClassifyTier(7) + TestParseEntriesFromSection(6) + TestScoreEntry(6) + TestPdfUrlDerivation(5) + TestPreprocessPdfText(5) + TestFetchDegrade(3) + TestPromptBuilders(4) + TestOutputBuilders(5) + TestRunOrchestrator(5 端到端：no_candidates/ok full_text/llm_failed 不产出 artifacts/degrade abstract_only/collector_failed) + TestKbDeepDiveShellGuards(9 源码级守卫：[SYSTEM_ALERT]/env-var heredoc 反模式/notify.sh 引用/set -euo pipefail/last_run/deep_dive topic/rsync forensics/fail-fast 顺序锁) |
