...
> 每次新会话开始时自动读取。当前版本：v37.4 / 0.37.4（2026-04-11）
...
| `ontology/` | **V36.1新增→V37.4升级** Ontology 独立子项目 — Tool Engine（81条声明式规则+推理引擎+**classify_tool_call语义分类**）+ **Governance Ontology v3.3**（**26不变式**+52可执行检查+**7元规则**+**MR-7 治理自观察**+**INV-GOV-001 summary 不吞 error**+**INV-DREAM-001/002 + INV-CACHE-002 Dream Map 预算+缓存契约**+验证深度三层模型+Phase 0元规则自主发现+MRD-LAYER-001深度盲区发现）+ governance_checker.py 执行引擎（V37.2+ silent error 修复：error 状态计入 failed_invs）+ 宪法6条 + 语义查询PoC + **Phase 2 shadow模式**(off→shadow→on) + 立场文章(EN+ZH) |
...
| `ontology/tests/test_dream_cache_stability.py` | **V37.4新增** Dream Fix A+B+C 回归测试（20 单测）：① Fix A cache-only fast path + SKIP_MAP_LOOPS 三处门控 ② Fix B 批次阈值 30/24000 ③ Fix C per-note content_hash 缓存 + 跨平台 md5 fallback + signal dedup ④ 动态 DREAM_TIMEOUT_SEC 预算 ⑤ flush_pending_batch 辅助函数语义 ⑥ Sources 缓存键保持不变 |
| `kb_dream.sh` | **V36新增→V37.4升级** Agent Dream v2 MapReduce 全量 KB 探索引擎（Phase1 Map 14源+286笔记逐一提取信号 → Phase2 Reduce 跨域深度分析；Map-Reduce 分离调度：00:00 Map预热缓存→00:40 Notes 预热→03:00 Reduce 跨域关联+推送；**V37.4 Fix A+B+C**：Reduce cache-only fast path（SKIP_MAP_LOOPS 门控）+ Notes 批次 15/12000→30/24000 + per-note 缓存（key=md5(content)）+ 动态预算 Map-Only 5400s / Reduce 3600s） |
