...
> 将任意大模型（Qwen / OpenAI / Gemini / Claude / Kimi / MiniMax / GLM / Doubao Seed 2.1 / **DeepSeek-V4-Pro**）一键接入 OpenClaw — 零第三方依赖、支持多模态、10 分钟跑通。

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)
[![Tests](https://img.shields.io/badge/tests-6384%20passed-brightgreen.svg)]()
...
> **Current version:** `v37.9.342` / `0.37.9.167` (2026-09-02) — see [`CLAUDE.md`](CLAUDE.md) for full changelog.
...
① Core data path
   User (WhatsApp + Discord)
     ...
     → LLM: Doubao Seed 2.1 Pro primary (PROVIDER env) → DeepSeek-V4-Pro GA / Doubao 2.1 via ai-tokenhub / Qwen3-235B fallback (12 providers, all OpenAI-compatible; GLM-5.3 coding on-demand via ?provider=glm5_coding)
...
| Remote LLM | — | 12 providers | Qwen3-235B / GPT-4o / Gemini 2.5 / Claude Sonnet / Kimi K2.5 / MiniMax M2.7 / GLM-5 / **Doubao Seed 2.1 Pro** (Volcengine Ark, primary) + **同模型经 ai-tokenhub** (`doubao` 槽位 = 跨平台冗余, V37.9.341) / **DeepSeek-V4-Pro** (w4a8 + 满血版 ai-tokenhub, V37.9.201/204/216) / **GLM-5.3 coding** (ai-tokenhub, on-demand, V37.9.254→290→339) |
...
| **Doubao Seed 2.1 Pro** (ai-tokenhub, plugin, `doubao` 槽位 V37.9.341) | doubao-seed-2-1-pro-260628 | Fallback #2（与 primary 同模型，跨平台冗余） | feature_verified（V37.9.342 E2E tool_calling+reasoning；text 待纯文本探针） |
...
python3 providers.py              # Markdown table (12 providers)
...
| `providers.py` | **V34** Provider Compatibility Layer — BaseProvider abstraction, 12 concrete providers (7 built-in + Doubao 2.1 ×2 platforms + DeepSeek×2 + GLM-5.3 coding plugins), ProviderRegistry, capability declaration, CLI matrix |
...
| File | Description |
|------|-------------|
| `docs/compatibility_matrix.md` | **V35** Provider compatibility matrix — 12 providers, verification status, degradation paths |
...
```bash
# Full regression (193 suites / 6384 tests / 0 fail; must ALL pass before push)
...
| **Fail-Plausible Detection Bench** | `docs/fail_plausible_bench.md` (community-runnable; defense 100% / FP 0% / honest held-out FN) | `python3 llm_observer_selfcheck.py --manifest` |
