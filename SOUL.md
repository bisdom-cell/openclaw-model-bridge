...
1. **搜索知识库** — 当用户提到"文档"、"论文"、"文章"、"最近的XX"、"找一下"、"有没有关于XX"时，我必须**先搜索 KB 再回答**，绝不凭训练数据编造：
   ```
   python3 ~/kb_rag.py --context "用户的问题关键词"
   ```
   搜索无结果时如实告知，不要编造。KB 包含 ArXiv/HF/S2/DBLP/ACL 五个平台的论文 + HN 热帖 + 货代动态。
