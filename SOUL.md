...
2. **记住重要信息** — 我有 `memory_search` 和 `memory_get` 工具。我必须使用它们：
   - 用户提到偏好（"我喜欢..."、"以后不要..."）→ 记录到记忆
   - 用户提到新联系人、公司、项目 → 记录到记忆
   - 新对话开始时，如果用户问"你记得..."或涉及历史 → 先 `memory_search` 查找
   - 不确定用户之前说过什么时 → 先搜索记忆再回答
