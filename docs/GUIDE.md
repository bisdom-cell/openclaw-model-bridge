...
- [Model Fallback / 模型降级](#model-fallback--模型降级v291)
...
## Knowledge Base System / 知识库系统（V29）
...
├── sources/        # Aggregated source files (arxiv, hn, freight, openclaw)
├── index.json      # Note metadata index
├── daily_digest.md # Daily KB summary (auto-generated by kb_inject.sh)
├── text_index/     # V29.3: Local embedding vectors (meta.json + vectors.bin)
└── mm_index/       # V29.1: Multimodal media vectors
```
...
|------|---------|-------------|
| Full-text search | `bash kb_search.sh "keyword"` | Keyword/tag/date/source filtering |
| Stats overview | `bash kb_search.sh --summary` | Entry count, source sizes, top tags |
| RAG search | `python3 kb_rag.py "query"` | Semantic search via local embedding |
| LLM context | `python3 kb_rag.py --context "query"` | Direct LLM-injectable format |
| Daily digest | `bash kb_inject.sh` | Generate daily summary |
| Deep review | `bash kb_review.sh` | Weekly LLM cross-note analysis |
| Deduplication | `python3 kb_dedup.py` | Exact/fuzzy dedup (dry-run) |

---

## Local AI / 本地 AI（V29.3）

### KB Text Embedding (Zero API Calls)
...
# Benchmark
python3 local_embed.py --bench
# → Single: ~10ms, Batch 100: ~500ms, Cross-lingual similarity: 0.920

# Build index
python3 kb_embed.py              # Incremental (skips unchanged files)
python3 kb_embed.py --reindex    # Full rebuild
python3 kb_embed.py --stats      # Index statistics

# RAG search
python3 kb_rag.py "Qwen3 模型架构"
python3 kb_rag.py --context "最近的AI论文"    # For LLM prompt injection
python3 kb_rag.py --json "shipping rates"    # For scripting
```

索引策略：400字/块，80字重叠，增量索引（文件hash去重），模型变更自动重建。

### Multimodal Memory (Gemini Embedding 2)

```bash
# Requires: pip3 install google-genai numpy
# Requires: GEMINI_API_KEY

python3 mm_index.py              # Index images/audio/video/PDF
python3 mm_search.py "猫的照片"   # Semantic search
python3 mm_search.py --stats     # Index statistics
```

Note: Local text models cannot embed images/audio/video, so multimodal memory still uses Gemini API.

本地文本模型无法处理多模态内容，因此图片/音频/视频索引仍使用 Gemini Embedding 2。
...
```bash
curl http://localhost:5002/health   # Cascade: proxy + adapter
curl http://localhost:5001/health   # Adapter only
