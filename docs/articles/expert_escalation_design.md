...
> **现状注（V37.9.341, 2026-09-02）**：expert 后端槽位模型现为 **Doubao Seed 2.1 Pro**（`doubao-seed-2-1-pro-260628` @ **ai-tokenhub**，见 `expert_escalation.py` 的 `EXPERT_DEFAULT_MODEL_ID`；后端槽位名已于 V37.9.345 中性化为 `default`，`doubao` 仅作兼容别名）；即 2.0 → 2.1 旗舰、Volcengine Ark → ai-tokenhub。本文的 Doubao Seed 2.0 Pro / Ark 定价等为设计时（2026-05）快照，不逐行改写，成本需按当前槽位模型与网关定价重估。
