...
> 版本：v37.9.96（**3657 tests / 102 suites / 781 governance checks / 85 invariants / 21 meta rules / 安全 95/100 / VERSION 0.37.9.68 不变** — V37.9.92-96 同 session 五阶段闭环展示 framework 从"靠记忆"升级为"机器化 + 制度化 + 自我演化"的关键拐点）。**今日 V37.9.96 一行 changelog**：周一开工选 #19（观察窗口型硬卡 LEVEL_6/ai_leaders 新账号要等 6/2-6/3）— 立 INV-PROXY-PLIST-ENV-001 守 com.openclaw.proxy.plist 必含 ARK_API_KEY + ARK_ENDPOINT_ID（V37.9.91 Doubao expert_escalate 真生产时 proxy plist 缺 ARK env 手动修，V37.9.54 INV-PLIST-ENV-001 只守 adapter plist 留下真缺口；ARK_ENDPOINT_ID 不是 api_key_env 故 adapter INV 覆盖不到）。MR-4，severity high，7 checks（5 file_contains + declaration/runtime python_assert，runtime 真读 proxy plist via plistlib，dev silent pass / Mac Mini 缺 ARK env fail-loud）。22 新单测含直接 exec yaml 真 runtime code 零 drift。**顺手修 audit_metadata 漂移**：total_invariants 83→85（V37.9.94 +1 INV-CROSS-ENV-PATH-001 漏记的 stale -1 + 本次 +1）+ total_checks 755→781（对齐 governance_checker 运行时权威计数）+ 新增 test_actual_invariant_count_matches_metadata 自洽守卫防再漂移。**上一条 V37.9.95**：周一原则 #32 用户视角观察 30min 第一次产 actionable feedback (V37.8.11 立 #32 后首次) — "建议加入更多 ai_leaders_x 观点, 尤其是更多 AI 大神不同观点". ai_leaders_x 19→31 跨 12 派别 (新增 Anthropic 派 3 / DeepMind / Safety 极致悲观 / 元老 Hinton / Fei-Fei Li / Robotics Abbeel / 开源 Soumith+HF / 架构创新 Yi Tay / 学术应用 Andrew Ng), MAX_PER_PERSON 5→3 + MAX_TOTAL 30→40 多元化优先. V37.8.4 INV-X-001 兼容 — WARN-on-zero-tweets 容错保留, 僵尸账号自动暴露 V37.9.96+ 清理. 14 新单测 + 反向 sabotage 守卫真有效. **昨日 V37.9.94 总结仍生效**：MR-15 framework 化 INV-CROSS-ENV-PATH-001 + V37.9.92 血案文档 (~370 行综合 case doc) — scanner 首扫即抓 V37.9.91 expert_escalation 隐藏第 5 次 near-miss 顺手修, framework 价值 commit 第一天就硬实证. **昨日 V37.9.93 总结仍生效**：Observer 采样 false-positive 修复 — smart head+tail sampling + CRITIQUE_SYSTEM 明示采样, MR-7 治理自观察最高境界 (Observer 自己发现自己 sampling 限制). **昨日 V37.9.92 总结仍生效**：V37.9.88 Mac Mini path bug + V37.9.84 status.json 双 silent failure 5 天闭环 (周一 Observer 数据驱动复盘 → 同 session 双 bug 修, MR-15 第 4 次演出). 反思文档"血案→反思→P0→framework 化预防"迭代机制硬实证.
