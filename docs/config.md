...
> 版本：v37.9.68-hotfix（**Mac Mini 5/15 03:00 cron 真激活验证发现 4 段被合并成 3 chunks 血案修复 — 用户视角原则 #13 第 20 次正向兑现 + V37.9.66 类别 B 设计假设错配** / **2962 tests / 86 suites / 660 governance checks** / 安全 95/100 / VERSION 0.37.9.68 不变 (hotfix 不 bump)。**今日 V37.9.68-hotfix 一行 changelog**：V37.9.68 Mac Mini 部署后用户开工立即报告 5/15 03:00 cron 真激活但 WhatsApp 收到 [3/3] 而非 [4/4]，单测/治理/preflight 全过. 根因: V37.9.68 changelog 说 "4 段 ## header 自然复用 V37.9.21 多窗口分片", 但 V37.9.21 实际是 `current+piece <= max_chunk 就合并` 算法. V37.9.68 4 段大小不均匀 (DEEP ~5000 / WIDE ~1700 / RADAR ~900 / 总览 ~400) 让 WIDE+RADAR (~2600) 被合并. MR-4 silent-failure 第 27 次新形态 — "设计假设与算法实现不一致". 修复 (MR-8 single-source-of-truth): kb_dream_helpers.py 加 split_dream_into_chunks 纯函数 (每段独立 + header 合并到第一段 + 超大段内部 \\n 切), kb_dream.sh 改用 helper. 12 新单测 + 反向验证两层守卫真有效 (sabotage 1 fail / 5 fail). INV-DREAM-MULTITHEME-001 内涵扩展 5 新 V37.9.68-hotfix check (helper 函数定义 + import + 真调用 + V37.9.68-hotfix marker + 反向反合并算法 python_assert). 接近真实数据模拟 → 4 chunks 完美匹配设计预期. **昨日 V37.9.68 总结仍生效**: Agent Dream 三阶推送重设计 - Qwen-BIM 连续几周血案闭环 + 6 因子涌现行为防御 (MR-4 第 26 次新形态)。**昨日 V37.9.67 总结仍生效**: P0 行动 1 兑现 INV-CROSS-OS-001 跨 OS quirk scanner. / **2944 tests / 85 suites** / 安全 95/100 / governance **77 inv / 655 checks / 19 meta rules** / **8 providers**, doubao fallback chain 第 1）。**今日 V37.9.68 一行 changelog**：用户视角原则 #13 第 19 次正向兑现 — WhatsApp 连续几周收到同款 Qwen-BIM 推送但单测/治理/preflight 全过. 6 因子叠加涌现行为 (用户 ⭐⭐⭐⭐⭐ 反复 × content-stable cache × LLM prompt 偏好 × mtime 倒序 × signal-hash dedup 不按主题 × PREV_THEMES 3 天太短 + prompt 后门). MR-4 silent-failure 第 26 次新形态 (V37.9.x 第一次涌现行为防御). 4 用户决策方案 C: 2 LLM 调用 (DEEP 独立 + WIDE+RADAR 合一) / 14 天 / WIDE 5+RADAR 5 / DEEP 必过. 新文件 kb_dream_helpers.py 8 纯函数 + 68 单测含血案场景回归. kb_dream.sh PREV_THEMES 3→14 天 + BANNED_THEMES_BLOCK + DEEP fail-fast (MIN_DEEP_CHARS=1200) + WIDE_RADAR 单 LLM 产 2 段 + 总览规则提取 + 4 段 ## header 多窗口. INV-DREAM-MULTITHEME-001 新增 (MR-4 high 19 checks) + INV-DREAM-003 架构迁移 CHUNK1/2/3 → DEEP/WIDE_RADAR. **昨日 V37.9.67 总结仍生效**: P0 行动 1 兑现 INV-CROSS-OS-001 跨 OS quirk scanner + 16 violations 全修齐. 反思文档 ontology/docs/reflections/2026-05-13_complexity_bug_taxonomy.md 沉淀 + 5 P0/P1/P2 行动. 反思文档"血案→反思→P0→framework 化预防"迭代机制硬实证.
