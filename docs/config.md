...
> 版本：v37.9.98（**3739 tests / 103 suites / 790 governance checks / 85 invariants / 21 meta rules / 安全 95/100 / VERSION 0.37.9.68 不变**）。**今日 V37.9.98 一行 changelog**：周二开工，多数候选（INV-OBSERVER / services flip）要 ~6/8 满一周观察硬卡 + LEVEL_6/ai_leaders 观察需 Mac Mini 数据 → 用户选 #1 来源可信度评级（observer 5/28 proposal #2，设计就绪+纯 dev 可做可测）。新 `source_credibility.py`（~230 行纯 stdlib，镜像 hallucination_guards.py PoC 模式）：5 档可信度 tier（🥇 学术同行评审 / 🥈 会议预印本 / 🥉 工业实践 / 📝 博客含 chaspark / 💬 社媒，按"引用断言无需独立验证可信程度"递减）+ SOURCE_CREDIBILITY 14 源映射（MR-8 单一真理源，test drift guard 守 ↔ jobs_registry 14 内容源 id+label 双匹配）+ get_credibility()/format_credibility_block() + CLI。wire 进 kb_dream.sh（DREAM_CREDIBILITY 同 DREAM_HG_GUARD 5 注入点）+ kb_evening_collect.py（build_evening_prompt 注入）。与 hallucination_guards 互补：HG 守内容真实性+跨域推论可信度，credibility 守出处权威性=双可信度轴。56 新单测 10 类 + 反向验证（sabotage chaspark tier / 删源 → drift guard fail，还原 56/56）。不立 INV（PoC，INV-SOURCE-CREDIBILITY-001 留 Mac Mini 一周观察后）。observer 从"被动评分"→"主动提议产品改进" = MR-7 治理自观察 + V37.9.83 AI Partnership 闭环。**上一条 V37.9.97**：V37.9.96 合并后用户问"下一个遗留任务"→ 选 #8 services_to_launchd Plan B 升级 machine_sync（convergence framework 最后一个 alert_only spec 收尾，自 5/4 起 4 周观察 zero drift + Mac Mini 6/1 实测 declared=3 observed=3 no drift）。核心 framework 增强：新 `_resolve_dry_run_for_spec(spec)` 让 spec 的 `dry_run_default` 字段从"纯文档性"升级"功能性"（三档优先级 CONVERGENCE_DRY_RUN env override > spec dry_run_default > framework 默认 False）—— services（dry_run_default:true，bootstrap blast-radius 高于 crontab）先 dry-run 观察一周，jobs/kb（false）保持 real-apply 向后兼容。新 `_apply_services_launchctl_bootstrap` per-service（查 services_registry plist，launchctl bootstrap missing labels 镜像 restart.sh V37.9.13，不 bootout=移除运行 service 非安全 auto-sync）。INV-CONVERGENCE-SERVICES-001 扩 +9 checks，governance v3.51→v3.52，26 新单测。**上一条 V37.9.96**：周一开工选 #19 — 立 INV-PROXY-PLIST-ENV-001 守 com.openclaw.proxy.plist 必含 ARK_API_KEY + ARK_ENDPOINT_ID（V37.9.91 Doubao expert_escalate 真生产缺口；ARK_ENDPOINT_ID 不是 api_key_env 故 V37.9.54 adapter INV 覆盖不到）。22 新单测含直接 exec yaml 真 runtime code 零 drift。顺手修 audit_metadata 漂移 total_invariants 83→85 + total_checks 对齐运行时权威计数 + 自洽守卫。**上一条 V37.9.95**：周一原则 #32 用户视角观察 30min 第一次产 actionable feedback (V37.8.11 立 #32 后首次) — "建议加入更多 ai_leaders_x 观点, 尤其是更多 AI 大神不同观点". ai_leaders_x 19→31 跨 12 派别 (新增 Anthropic 派 3 / DeepMind / Safety 极致悲观 / 元老 Hinton / Fei-Fei Li / Robotics Abbeel / 开源 Soumith+HF / 架构创新 Yi Tay / 学术应用 Andrew Ng), MAX_PER_PERSON 5→3 + MAX_TOTAL 30→40 多元化优先. V37.8.4 INV-X-001 兼容 — WARN-on-zero-tweets 容错保留, 僵尸账号自动暴露 V37.9.96+ 清理. 14 新单测 + 反向 sabotage 守卫真有效. **昨日 V37.9.94 总结仍生效**：MR-15 framework 化 INV-CROSS-ENV-PATH-001 + V37.9.92 血案文档 (~370 行综合 case doc) — scanner 首扫即抓 V37.9.91 expert_escalation 隐藏第 5 次 near-miss 顺手修, framework 价值 commit 第一天就硬实证. **昨日 V37.9.93 总结仍生效**：Observer 采样 false-positive 修复 — smart head+tail sampling + CRITIQUE_SYSTEM 明示采样, MR-7 治理自观察最高境界 (Observer 自己发现自己 sampling 限制). **昨日 V37.9.92 总结仍生效**：V37.9.88 Mac Mini path bug + V37.9.84 status.json 双 silent failure 5 天闭环 (周一 Observer 数据驱动复盘 → 同 session 双 bug 修, MR-15 第 4 次演出). 反思文档"血案→反思→P0→framework 化预防"迭代机制硬实证.
> OpenClaw Gateway：2026.3.13-1（当前部署，继续 hold）| 上游最新：**v2026.6.1**（2026-06-03 发布）。**第五次评估（2026-06-08，当前数据 WebFetch 实证）**：用户主动要求充分评估风险（已推迟 3 个月）。裁决——3 个月 hold 有理（#73358 无 OpenAI key 时 Codex silent default = 静默业务中断，真 dealbreaker 直到 v2026.4.27 修复）。当前数据：(a) #59265 Agent 不可见**再确认仍 closed 无 PR / 无 fix**（G2 门不通过）(b) 6.x 又加 **SQLite migration + plugin policy overhaul 2 个新 breaking** → 瞄准 **v2026.4.27** 可避开全部 4.29/5.x/6.x breaking，比 5/5 时更有理 (c) 版本差距 tripwire likely 已触发（3.13→6.1）。**用户决策：继续 hold 到文档 6/20 时间表**（v2026.4.27 已 ~40 天稳定期技术成熟，但保守纪律——weekend 回滚窗口 + 6/15 最终前置验证 + 升级是 Mac Mini SSH 操作禁 AI 执行）。最大残留风险 #59265 显示 bug 只能升级后 WhatsApp E2E 观察 + 30 秒回滚。详见 `docs/gateway_upgrade_eval_v2026.4.md` 第十四节。LAST_EVAL_DATE 更新到 2026-06-08。
---
## 一、系统架构（V28.1 四层架构）
