...
## 九、实施路径（分阶段，今日仅 Stage 0）

| Stage | 内容 | 成功定义 | 复杂度预算 |
|-------|------|----------|------------|
| **Stage 0** ✅ | 本设计文档（5 子目标拆解 + 锚点 + 边界） | design doc 入库，方向可执行 | files +1 (doc) |
| **Stage 1** | ground-truth 标注集：22 corpus 打 §4.2 schema 标签（单表，一物一形）+ 5 黄金种子 | 标注表入库，Category A/B 划分清晰，observer_in_scope 诚实划界 | files +1 (标注表), 零 runtime |
| **Stage 2** | 确定性 pre-filter（S1-S5，零 LLM，复用 seeds）+ 单测 + sabotage 守卫 | S1-S5 对 5 黄金 case 命中、干净输出不命中、sabotage 改坏即漏检 | +1 module + test |
| **Stage 3** | LLM-judge Layer 2（FAIL_PLAUSIBLE_SYSTEM，复用 call_llm_critique）+ 注入 LEVEL_6/credibility | verdict 结构化 + 引用证据 + 防 V37.9.93 sampling 幻觉 | 扩既有 infra |
| **Stage 4** | self-validation harness（复用 adversarial_chaos_audit 模式，OBS-A/B 场景）+ scorecard | defense rate / FN / FP / calibration 可度量；Category A →100% | 扩既有 harness |
| **Stage 5** | 接入 `daily_observer.run()`（detect_fail_plausible，§6.1）+ score/status 向后兼容扩展 | daily_observer 次日报告含 fail_plausible 段，零下游破坏，Mac Mini E2E | 扩既有 job, 净 runtime ≈ 1 函数 |
| **Stage 6** | bench 化（reliability_bench 扩展为 silent-failure/fail-plausible 检测 bench，社区可跑）+ 论文 #2 草案 | 别人能 `pip`/clone 跑 bench；论文 #2 描述→预测的 scorecard | 广度交付 |

**今日交付 = Stage 0 本文档**。后续 Stage 由独立 session 推进（每 Stage 走完整收工清单 + Mac Mini E2E，原则 #6/#9/#29）。
