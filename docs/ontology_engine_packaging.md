...
| ~~MRD 扫描文件名/glob 模式 (`jobs_registry.yaml` / `notify.sh` / `preflight_check.sh` / 诊断白名单 / 7 glob 形状 / push-route 白名单)~~ ✅ **V37.9.126 chunk-3b + V37.9.127 chunk-3b.2 全修复** | `governance_checker.py::_discover_*()` | **全部 MRD 扫描模式已 config-inject**：`_load_mrd_patterns()` 读 `governance_ontology.yaml::mrd_scan_patterns` (单文件 chunk-3b + 7 glob-set + push-route 白名单 chunk-3b.2)，`_mrd_globs()` helper；消费方可 override 自己的项目结构 (如 `skills/**/*.py`)；缺段/键 → `_MRD_DEFAULTS` (bridge 字节级一致)。WeatherBot demo 端到端验证 (`weatherbot_jobs.yaml` + `skills/**/*.py`)。 | ✅ 全修。byte-identical 验证 (89 inv/14 MRD discovery + 12 glob-依赖计数全不变) + 40 单测 + 反向 sabotage + demo。MRD 扫描器全部项目无关化 (终极目标可包化粒度达成)。 |
...
| 5 (Phase 5 终态) | 真 sdist/wheel 构建 + `pip install` (非 editable) 冒烟 + 发布决策 (PyPI 名 / license / readme) + 版本治理 (semver) | — | ✅ 已完成 (V37.9.133 build/install 冒烟 + V37.9.137 PyPI 发布) |
| **6 (本次)** | **仓库外 dogfood (外部评审2 P2(c))**：把 V37.9.137 一次性 PyPI 冒烟升级为 CI 回归守卫 — `examples/external_dogfood/` 离线 build wheel → 隔离 venv 装 wheel（无 PYTHONPATH 回 monorepo）→ 全新 LibraryBot toy 项目复制出仓库到 /tmp → 经 `import ontology_engine`（去泛化名）+ console scripts 审计 + 反向验证（无注入 → wheel bundled 默认）。17 单测（source guards 恒跑 + wheel E2E skipUnless 工具链）+ sabotage 反向验证（toy 源破坏 → audit 检出）。证发布的引擎可被第三方零 monorepo 访问消费。 | 低（纯新增，证据刷新非框架改动） | ✅ 完成 (V37.9.148) |
