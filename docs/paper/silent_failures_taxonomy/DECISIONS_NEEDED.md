...
**第二轮已完成（2026-06-11 同日，draft v0.2）**：✅ TBV 引用核实（4 个 arXiv 作者 + Ezell AIES venue）✅ 三类 related work 补齐（hallucination survey → §2.3 / chaos engineering + AIOps → 新 §2.4，13 条 verified references）✅ Fig.3 D1 幻觉链 ASCII 图入 §4.4。

仍可自主推进（无需决策）：LLM-as-judge/observer 文献一类（§5.2 支撑）/ snippet 级引用对 arXiv abs 页终核（需可达环境，dev 403）/ 正文语言第三轮精磨 / data_inventory 投稿前重新对表。**venue 决策后**：LaTeX 转换 + Fig.1/2/4/5 正式绘制（Fig.3 已有 ASCII 底稿）。
