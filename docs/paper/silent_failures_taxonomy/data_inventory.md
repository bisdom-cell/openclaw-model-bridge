...
> **✅ 终版归档（2026-06-15）**：论文已发表为 [arXiv:2606.14589](https://arxiv.org/abs/2606.14589)（cs.SE）。本表为投稿前对表终版，所有数字冻结于 study-cutoff 2026-06-11（匹配已发表的 LaTeX 版本）；引用终核 6 篇 arXiv 官方 API 作者列表 100% 一致已清零（见末尾 References 段）。已发表论文是冻结快照，活的仓库统计（README 徽章 / VERSION 等）会继续演进——两者允许不同（V37.9.144 原则：活 README 与冻结论文允许不同；本表是论文的 convergence spec，投稿后不再追仓库当前态）。
