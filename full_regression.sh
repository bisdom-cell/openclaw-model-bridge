...
run_suite "pa_coupling (V37.9.249-251 H1-B PA 硬编码守卫 — A1 kb_*.sh 默认 \$HOME/.kb + D3 35 脚本 TZ=\${SYSTEM_TZ:-…} + A2 diagnose \$HOME/.openclaw + B3 全仓 122 文件拦截新写 /Users/<name> 个人路径, docs/pa_coupling_inventory.md)" "python3 test_pa_coupling.py"
...
run_suite "v37_9_218_fallback_order (V37.9.218 FALLBACK_ORDER 显式有序 fallback 链 + capability-aware vision fallback — 替代单槽一物一形)" "python3 test_v37_9_218_fallback_order.py"
...
run_suite "v37_9_268_security_badge_source (V37.9.268 徽章源 quality.security_score vs full_regression 实时 security 解耦 — dev git remote http-proxy artifact 致假 doc-drift 根因修复)" "python3 test_v37_9_268_security_badge_source.py"
...
run_suite "v37_9_288_proxy_honest_output (V37.9.288 对抗审计批次2 — 参数 null-归一/fix_date_cols 打通/SystemExit 收编/搜索坏了≠没搜到/expert SOUL 渲染/per-step 诚实)" "python3 test_v37_9_288_proxy_honest_output.py"
...
run_suite "v37_9_306_evening_llm_resilience (V37.9.306 kb_evening llm_failed 告警 — 客户端超时紧于服务端预算 120→300s + 同族唯一无重试 job 补 retry=1, 共享 call_llm retries 默认 0 零回归)" "python3 test_v37_9_306_evening_llm_resilience.py"
...
run_suite "v37_9_273_override_circuit_breaker (V37.9.273 ?provider= override 请求不污染默认 primary 断路器 — GLM chat 路由 audit finding)" "python3 test_v37_9_273_override_circuit_breaker.py"
...
run_suite "llm_observer (V37.9.195 研究攻关 #1 Stage 2 — 确定性 Layer 1 pre-filter S1-S5 + 5 黄金 case ground-truth tie + 干净输出 FP=0 + sabotage)" "python3 test_llm_observer.py"
run_suite "llm_observer_selfcheck (V37.9.197 研究攻关 #1 Stage 4 — 自验证 harness scorecard: defense 100%/FP 0%/Category B held-out FN + sabotage 证 detector load-bearing)" "python3 test_llm_observer_selfcheck.py"
...
run_suite "ai_leaders_bsky (V37.9.111 ai_leaders 加 Bluesky 实时短观点维度 — getAuthorFeed JSON + 复用 blogs 管道 + parser 行为守卫)" "python3 test_ai_leaders_bsky.py"
...
run_suite "v37_9_336_upgrade_sop_currency (V37.9.336 第八次评估 — SOP 硬编码回滚版本血案 + 30 秒无损承诺退役 + 评估↔SOP 跨节前置契约 + 原则 #35 命令块无注释; V37.9.350 第九次评估落地 + 19.8 协议重标定行为级真跑 + 最新评估节动态派生)" "python3 test_v37_9_336_upgrade_sop_currency.py"
run_suite "v37_9_337_finance_llm_diagnosability (V37.9.337 finance_news LLM 失败可定性 — curl 退出码不再被 || true 吞掉 + 三层分类对齐兄弟 job + 短内容不谎称调用失败)" "python3 test_v37_9_337_finance_llm_diagnosability.py"
run_suite "v37_9_338_cron_line_canonical (V37.9.338 canonical crontab 行 — 治理 INV-CRON-003 判据前移 dev CI + 手写行退役, 血案: check_upgrade 缺 bash -lc 连报两天)" "python3 test_v37_9_338_cron_line_canonical.py"
...
run_suite "slo_benchmark (V37.9.99 三态 + V37.9.143 四态 N_A/双窗口/阈值正文)" "python3 test_slo_benchmark.py"
...
# V37.9.241: 三个暗测试补注册 — 从未进 full_regression 导致其中两个期望静默腐烂
# (test_engine 3 fail: V37.9.91 +expert_escalate 后 2→3/18→19 无人知;
...
# V37.9.329: Dream 素材覆盖面 — 位置性窗口 → 内容感知取样 + 跨夜轮转（增长维度）
if [ -f test_v37_9_329_dream_material_coverage.py ]; then
    run_suite "v37_9_329_dream_material_coverage (V37.9.329: sources 月份分层取样替换尾部窗口 + max-min 公平预算修字母序丢源 + 日序轮转让覆盖率与语料规模无关 + 跨时间对照 prompt)" "python3 test_v37_9_329_dream_material_coverage.py"
fi
...
if python3 gen_compat_matrix.py --check >/dev/null 2>&1; then
    ...
    echo "❌"
    python3 gen_compat_matrix.py --check 2>&1 | tail -5
    FAIL=$((FAIL + 1))
    FAILED_SUITES+=("compat matrix drift")
fi
echo ""
...
if [ -f status_update.py ]; then
        ...
        if [ -f security_score.py ]; then
            ...
            if [ -n "$SEC_SCORE" ]; then
                # V37.9.268: 不再用环境敏感的实时 security 覆盖徽章源 quality.security_score
                # (gen_readme_badges:104 读它做 doc header 徽章)。根因: security_score 传输安全
                # 的 Git 协议子项跑 `git remote -v` — dev 沙箱 remote 是 http-proxy → 0 分,
                # 生产 SSH → 5 分 ⇒ dev 95 vs 生产 98 = 纯 dev 环境 artifact 非真实安全差异。
                # 每次 full_regression 用 dev 值覆盖徽章源会让 dev doc-drift 假失败
                # (V37.9.243/245 长期"偶发失败观察项"根因, V37.9.267 厘清)。
                # 修复: quality.security_score 作稳定发布态徽章源(生产真变时手动 --set),
                # 只记 health.security_score 作环境敏感诊断值(不驱动徽章)。
                python3 status_update.py --set health.security_score "${SEC_SCORE}/${SEC_MAX} (${SEC_PCT}%)" --by full_regression 2>/dev/null
                echo "   security_score=${SEC_SCORE}/${SEC_MAX} (${SEC_PCT}%) [记 health; 徽章源 quality 保持发布态]"
            fi
        fi
