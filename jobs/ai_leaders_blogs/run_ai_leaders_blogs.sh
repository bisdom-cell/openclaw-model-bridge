...
# AI 大神长文观点监控 (V37.9.108 — ai_leaders 从 X 转向博客/Substack RSS)
# 目的: 收集 AI 领域顶尖学者专家的长文观点/不同意见, 结合项目深度分析发现机会点。
# 背景: ai_leaders_x (X Syndication) 因 429 + 快照冻结退化产 ~0 (V37.9.101/102/103)。
#   非 X 渠道解法: 大神在 blog/Substack 发长文推理观点 (远优于 280 字推文, 更适合深度分析)。
#   复用 rss_blogs 已验证管道 (RSS + LLM 6 字段 + 🎚️ 项目对齐评分 + 双通道 + KB),
#   feed 列表选不与 rss_blogs 重叠的大神, 偏重"不同意见"(含 contrarian)。
# ⚠️ feed 可达性必须 Mac Mini 首跑验证 (原则 #33 + 反馈 #1 chaspark 教训:
#   dev sandbox 网络 403 不代表 feed 死)。每 feed FAIL-OPEN, 死链 log WARN + 跳过不杀 job。
# 每天 1 次 (13:30 HKT) 由系统 crontab 触发。
set -eo pipefail
...
# 防重叠执行
LOCK="/tmp/ai_leaders_blogs.lockdir"
mkdir "$LOCK" 2>/dev/null || { echo "[ai_leaders] Already running, skip"; exit 0; }
trap 'rmdir "$LOCK" 2>/dev/null' EXIT

OPENCLAW="${OPENCLAW:-/opt/homebrew/bin/openclaw}"
JOB_DIR="${HOME}/.openclaw/jobs/ai_leaders_blogs"
CACHE="$JOB_DIR/cache"
KB_SRC="${KB_BASE:-$HOME/.kb}/sources/ai_leaders_blogs.md"
...
send_alert() {
    local reason="$1"
    local msg="[SYSTEM_ALERT] ai_leaders_blogs LLM 失败
时间: $TS
原因: $reason
降级处理: 今日未推送 AI 大神观点 (避免占位符污染)
...
test -f "$KB_SRC" || echo "# AI 大神长文观点 (博客/Substack)" > "$KB_SRC"

# ── AI 大神博客/Substack 源 (V37.9.108) ───────────────────────────────
# 格式：name|feed_url|label
# 策展原则: (1) AI 领域顶尖个人学者/专家的长文观点 (2) 不同意见/流派 (含 contrarian)
#   (3) 不与 rss_blogs 4 个源重叠 (科学空间/Lil'Log/Simon Willison/Latent Space)。
# ⚠️ feed URL 是 candidate (标准 Substack /feed + GitHub Pages /index.xml 模式),
#   Mac Mini 首跑验证 — 死链会 log "WARN: X RSS 抓取失败，跳过" (FAIL-OPEN), 之后剪枝。
RSS_FEEDS=(
    # ── 怀疑派 / 不同意见 (contrarian, 用户要的"不同意见"核心) ──
    "Marcus on AI|https://garymarcus.substack.com/feed|Gary Marcus(神经符号怀疑派,反LLM-AGI)"
    "AI Snake Oil|https://www.aisnakeoil.com/feed|Narayanan&Kapoor(批判性AI/炒作祛魅)"
    # ── 政策 / 行业观察 ──
    "Import AI|https://importai.substack.com/feed|Jack Clark/Anthropic(政策+研究综述)"
    # ── 研究深读 / 训练洞察 ──
    "Ahead of AI|https://magazine.sebastianraschka.com/feed|Sebastian Raschka(ML研究深读)"
    "Interconnects|https://www.interconnects.ai/feed|Nathan Lambert(RLHF/开源模型/post-training)"
    # ── 系统 / 工程实践 ──
    "Eugene Yan|https://eugeneyan.com/rss/|Eugene Yan(应用ML/LLM系统设计)"
...
TOTAL_NEW=0
# V37.9.238 (audit F follow-up): 每 feed 抓取失败计数 (镜像 ontology_sources V37.9.227)。
# 此前每 feed FAIL-OPEN WARN+skip 无计数 → 全源宕仍写 status:ok = watchdog 静默。
FETCH_ERRORS=0
...
try:
    ...
    try:
        ...
        print(f"[ai_leaders] ERROR: {feed_name} XML解析失败", file=sys.stderr)
...
# HTML 不触发 ParseError 却 0 item → 静默 status:ok）。根元素白名单闭合此盲区。
_root_tag = root.tag.split('}')[-1].lower()
if _root_tag not in ('rss', 'feed', 'rdf', 'channel'):
    print(f"[ai_leaders] ERROR: {feed_name} 根元素 <{_root_tag}> 非 RSS/Atom（反爬 HTML?）", file=sys.stderr)
    sys.exit(2)
...
print(f"[ai_leaders] {feed_name}: {new_count} 篇新文章", file=sys.stderr)
PYEOF
...
if [ "$TOTAL_NEW" -eq 0 ]; then
    # V37.9.238 (audit F follow-up): 全源抓取失败 ≠ 平静无新文章 (镜像 ontology_sources
    # V37.9.227)。全部 RSS 源失败 → fetch_failed + exit 1 (watchdog 显式 case 告警)。
...
echo "[ai_leaders] 共 ${TOTAL_NEW} 篇新文章"
...
prompt = """你是 AI 学者观点深度分析师 (兼 OpenClaw 项目对齐评估师)。下文是 AI 领域顶尖学者/专家在个人博客或 Substack 发表的长文观点。提炼其核心立场与独特视角, 输出 6 字段中文分析:

📌 中文标题: 信达雅翻译, 不超过 25 字 (如原文已是中文则简化精炼)
🔑 核心要点: 3-5 条 bullet, 每条 1 句 ≤ 60 字, 列出作者最重要的论点/主张/判断
💡 关键洞察: 重点揭示 ① 作者的独特立场/观点 (尤其与主流共识的分歧或不同意见) ② 论证逻辑/方法论 ③ 与行业趋势/其他学者观点的关联或对立 ④ 局限性或可争议处
   长度按评级动态调整: ⭐⭐⭐→100-150字 / ⭐⭐⭐⭐→250-400字 / ⭐⭐⭐⭐⭐→500-800字 (旗舰级观点充分展开)
        ...
        if fields['alignment']:
            ...
            if _validate_alignment_score and _concepts and _extract_star_count and _format_validation_marker:
                try:
                    ...
                    print(f"[ai_leaders] V37.9.51 rule_check 失败 article={i}: {_e} (FAIL-OPEN)", file=sys.stderr)
            msg_lines.append("")
...
print(f"[ai_leaders] 消息组装完成: {len(articles)} 篇 (LLM 解析成功 {llm_ok_count}, degraded {degraded_count}, 高对齐 {high_alignment_count})", file=sys.stderr)
PYEOF
...
# 按 "\n---\n" 切分文章块, 第一块是 header "🧠 AI 大神观点 (date)"
blocks = re.split(r'\n---\n', content)
...
for i, chunk in enumerate(chunks):
    if total_parts > 1:
        ...
        if i == 0:
            chunk = chunk.replace(f"🧠 AI 大神观点 ({day})",
                                  f"🧠 AI 大神观点 [1/{total_parts}] ({day})", 1)
        else:
            chunk = f"🧠 AI 大神观点 [{i+1}/{total_parts}] ({day}) (续)\n\n" + chunk
...
${SUMMARY}"
    bash "$KB_WRITE_SCRIPT" "$CONTENT" "ai-leaders-blogs" "note" 2>/dev/null || true
    echo "[ai_leaders] KB写入完成"
fi
