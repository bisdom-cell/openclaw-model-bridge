...
# V37.9.234 (observer 2026-07-03 提案 #1): 标题推断粒度收敛 — 原 "按评级 100-800 字"
# 的长度扩展要求结构性逼迫 LLM 从纯标题编造细粒度方法细节 ("推测采用预训练的基础模型"
# "可能引入对比学习或适配器模块", observer [MED] finding), 改固定 80-150 字方向性解读
# + 严格约束显式禁止细粒度技术路径猜测。"(基于标题推断)" 诚实标注保留 (V37.9.213 F4)。
prompt = """你是 AI/CS 论文深度分析师 (兼 OpenClaw 项目对齐评估师)。对以下论文输出 6 字段中文分析:

📌 中文标题: 信达雅翻译, 不超过 25 字 (技术术语保持精确)
🔑 核心贡献: 2-4 条 bullet, 每条 1 句 ≤ 60 字 (只写标题字面可支持的方向性判断, 不虚构细节)
💡 关键方法: 基于标题与发表会议的方向性解读 (研究领域 / 要解决的问题 / 标题明确点名的方法大类)
   长度固定 80-150 字, 不随评级扩展 — 标题无法支撑更长的真实分析
   注意: DBLP 不提供摘要, 此处仅做方向性解读, 必须显式标注 "(基于标题推断)"
...
- 推断粒度收敛: 严禁细粒度技术路径猜测 ("推测采用XX模型/架构"、"可能引入XX模块/机制/损失函数" 类表述一律禁止) — 只允许标题字面直接支持的方向性判断
...
if [ "$TOTAL_LEN" -le 8000 ]; then
    ...
    # V37.9.171: 主推/分块走 notify.sh（微信→用户 + Discord #papers + 重试/队列），退役裸 whatsapp+discord 对
