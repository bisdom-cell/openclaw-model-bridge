...
#
# V37.9.51 6 字段 + rule_check 升级 (V37.9.45 hf_papers / V37.9.50 semantic_scholar 同款 Opportunity Radar #2):
#   - 第 6 字段 🎚️ 项目对齐度 (⭐ × N + 一句话原因, OpenClaw 5 档评分指南)
#   - parse 函数加 alignment 字段 + lazy import project_alignment_scorer
#   - rule_check 验证 LLM ⭐ vs repo 描述 keyword 一致性
#   - 末尾"本轮高对齐 ⭐≥4: N/M 个仓库"统计
...
prompt = """你是 AI 技术深度分析师 (兼 OpenClaw 项目对齐评估师)。对以下 GitHub 热门仓库输出 6 字段中文分析:
...
def parse_6field_output(content):
    ...
    返回 dict: cn_name / highlights / insight / practice / rating / alignment
...
try:
    ...
    print("[gh_trending] V37.9.51 project_alignment_scorer 加载成功 (rule_check 启用)", file=sys.stderr)
except Exception as _e:
    print(f"[gh_trending] V37.9.51 project_alignment_scorer 缺失或失败: {_e} (rule_check 跳过, FAIL-OPEN)", file=sys.stderr)
        ...
        if fields['alignment']:
            ...
            if _validate_alignment_score and _concepts and _extract_star_count and _format_validation_marker:
                try:
                    ...
                    if llm_stars > 0:
                        # rule_content = full_name + description + topics (GitHub 项目, V37.9.47 hf_papers 同款模式适配)
                        rule_content = repo.get('full_name', '') + ' ' + (repo.get('description') or '') + ' ' + ' '.join(repo.get('topics', []) or [])
                    ...
                    print(f"[gh_trending] V37.9.51 rule_check 失败 repo={i}: {_e} (FAIL-OPEN)", file=sys.stderr)
            msg_lines.append("")
...
if total_repos > 0:
    msg_lines.append(f"━━━ 本轮高对齐仓库 (项目对齐度 ⭐≥4): {high_alignment_count}/{total_repos} 个 ━━━")
    msg_lines.append("")

with open(msg_file, 'w', encoding='utf-8') as f:
    f.write('\n'.join(msg_lines))

print(f"[gh_trending] 消息组装完成: {len(repos)} 个 repo (LLM 解析成功 {llm_ok_count}, degraded {degraded_count}, 高对齐 {high_alignment_count})", file=sys.stderr)
PYEOF
