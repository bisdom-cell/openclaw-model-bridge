...
if [ "$TOTAL_LEN" -le 8000 ]; then
    ...
    # V37.9.171: 走 notify.sh（微信→用户 + Discord #papers + 重试/队列），退役裸 whatsapp+discord 对
    if notify "$MSG_CONTENT" --topic papers >/dev/null 2>"$SEND_ERR"; then
...
PYEOF
    ...
    for chunk_file in "$WA_CHUNK_DIR"/*.txt; do
        ...
        # V37.9.171: 走 notify.sh（微信 + Discord #papers + 重试/队列）
        if notify "$CHUNK_CONTENT" --topic papers >/dev/null 2>>"$SEND_ERR"; then
