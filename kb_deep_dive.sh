...
# V37.9.63: ERR trap 走公共 helper (MR-8 抽公共 — 之前 V37.9.61 inline _kb_deep_dive_fatal_handler)
HELPER_DIR="$(cd "$(dirname "$0")" && pwd)"
if [ -f "$HELPER_DIR/cron_monitor_fatal_handler.sh" ]; then
    # shellcheck disable=SC1091
    source "$HELPER_DIR/cron_monitor_fatal_handler.sh"
    CRON_FATAL_LABEL="kb_deep_dive"
    CRON_FATAL_LOG="$HOME/kb_deep_dive.log"
    CRON_FATAL_BASH_X="bash -x ~/kb_deep_dive.sh"
...
send_wa_parts_via_notify() {
    for chunk_file in "$WA_CHUNK_DIR"/*.txt; do
        ...
        if notify "$WA_SEGMENT" --channel whatsapp --topic deep_dive >/dev/null 2>&1; then  # V37.9.182: openclaw-weixin → whatsapp 回退（V37.9.179 回退默认通道时漏改本行；微信仅 48h 客服窗口内可投递、窗口外静默丢失，不适合无人值守 cron；WhatsApp 已恢复可靠。Discord 由下方 DISCORD_MSG 单发，避免重复）
