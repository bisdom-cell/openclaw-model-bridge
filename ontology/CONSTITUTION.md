...
硬编码系统（成熟、久经生产验证）
  ...
  └── 6504 个测试（2026-09）→ 组件行为验证

本体系统（新兴、声明式、可推理）
  ├── tool_ontology.yaml       → 工具规则声明（16 builtin + 1 prefix + 3 custom + 11 policies）
  ├── governance_ontology.yaml → 91 不变式 + 23 元规则 + 14 MRD（2026-09）
...
|---|------|-------------------|-------------------|---------|
| F1 | **工具数量** | ≤20：手动列举可控 | >30：人工 review 遗漏率上升；>50：手动维护不可行 | 16 个白名单 + 3 自定义（低） |
