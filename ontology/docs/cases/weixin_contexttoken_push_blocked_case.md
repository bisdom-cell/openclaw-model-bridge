...
## ⚠️ V37.9.182 修正（2026-06-22）：真闸门是 48h 客服窗口，**不是** contextToken —— V37.9.179 结论被证伪

> **本案 V37.9.179 的核心结论「无 token → 微信永远丢弃 / 架构上做不了推送」过头了，被 Mac Mini 生产日志证伪。** 下面分析的"第六层机制"和"修复"段保留作历史记录，但以本修正段为准。

**触发**：2026-06-21/06-19 用户**连续收到**微信「🔬 每日深度」推送（cron 发起、非交互回复），直接矛盾 V37.9.179「cron 推送零到达微信」。

**铁证（2026-06-21 Mac Mini `/tmp/openclaw/openclaw-*.log`）**：

| 时间 | 日志 | 含义 |
|------|------|------|
| 13:43/13:45 | `outbound ... contextToken=AARzJW…(len=136)` `hasContextToken=true` | 用户白天与 bot 交互，PA 回复**有 token**（路径 A 不变） |
| **22:30** | `sendWeixinOutbound: contextToken missing ... sending without context` (WARN) | deep_dive cron 发起、**无 token**——**却投递成功**（用户收到 06-21 每日深度） |
| `channels status` | `openclaw-weixin ... in:29h ago` | 用户最后入站 **29h < 48h → 客服窗口开着** |

**修正后的机制**：真正的投递闸门是微信**48h 客服会话窗口**，不是 contextToken。
- contextToken 决定**会话串接（threading）**，**不**决定投递权限——V37.9.179 把两者混为一谈（本案"第四层"已考虑过 48h 窗口，却被错误否定为"比窗口更彻底"，那个否定就是过头处）。
- 窗口开（用户近 48h 内有入站）→ 无 token 的 CLI outbound **照样投递**；窗口关 → 丢弃。
- V37.9.179 那次"3h 零到达"很可能在**窗口关闭**期；用户近期天天测微信把窗口顶开，deep_dive 才落进窗口。

**为什么只有 deep_dive 到微信**：默认 `NOTIFY_CHANNELS="whatsapp,discord"` 不含 weixin；全仓只有 `kb_deep_dive.sh` 硬编码 `--channel openclaw-weixin`（V37.9.174 遗留，**V37.9.179 回退默认通道时漏改这一行**）。V37.9.182 已把它回退为 `--channel whatsapp`。

**净结论（精炼而非反转）**：微信 CLI 推送**在 48h 客服窗口内可投递、窗口外静默丢失**。投递依赖用户持续交互维持窗口 → **对无人值守 cron 仍不可靠**，不适合做内容主推（V37.9.179 的"不适合 cron 主推"方向**对**，"永远做不了/架构上不可能"的**绝对化错**）。默认通道维持 discord + WhatsApp 不变。

**元教训（正中反馈 #1：环境受限就下"不可能"结论）**：V37.9.179 从一次 3h 观察（恰逢窗口关闭）外推出"永远/架构上做不了"的绝对结论，是 **fail-plausible 的镜像**——这次不是系统报告假成功，而是**诊断者自己下了一个 plausible 但过头的结论**。修正方法：绝对化结论（"永远""不可能""架构上做不了"）必须有跨多条件的证据；单次观察只能下"在 X 条件下未投递"。本案"第四层"列出了正确假设（48h 窗口）却选了更激进的否定——记住：**当数据同时支持"保守解释"和"激进解释"时，激进解释需要更高举证门槛**。
