"""V37.9.52/53/54/55 — Doubao Seed 2.0 Pro Provider → V37.9.290 平台切换 ai-tokenhub
   ...
   → V37.9.342 Mac Mini E2E (ai-tokenhub 路径): tool_calling + reasoning 2 项实测 →
                 feature_verified; verified_text 刻意仍 False (见下方"为什么 text 没验证")
...
- 🔴 **为什么 text 没验证**: 该轮探针带 tools, 模型选择调工具而非作答 → `content` 为空 +
  finish_reason=tool_calls。本项目对 verified_text 的既定证据标准是「有 content +
  finish_reason=stop」(doubao_21 V37.9.217 / deepseek_full+glm5 V37.9.340 皆如此), 空 content
  够不着 → **verified_text 保持 False**, 待一次不带 tools 的纯文本探针补齐 (原则 #23:
  不把「模型工作正常」笼统当成「每一维都验证过」)。
...
class DoubaoSeedProvider(BaseProvider):
    ...
    capabilities = ProviderCapabilities(
        ...
        verified_text=False,         # 🔴 探针带 tools → content 空 + finish_reason=tool_calls,
                                     #   够不着本项目 verified_text 的证据标准 (见 docstring)
        ...
        tier_note="2026-09-02 槽位更正 doubao-2.1 @ ai-tokenhub (V37.9.341) 后 E2E 半升档 (tool_calling+reasoning, V37.9.342)",
        tier_evidence="ai-tokenhub E2E 探针 2026-09-02 (V37.9.342, model=doubao-seed-2-1-pro-260628): "
                      ...
                      "🔴 verified_text 刻意保持 False —— 该轮带 tools 故 content 为空, 够不着本项目 "
                      "verified_text 的证据标准 (有 content + finish_reason=stop), 待纯文本探针补齐；"
