...
- **vision 不声明 (V37.9.347 实测坐实)**: V37.9.218 capability-aware vision fallback 会把
  image 请求路由到声明 vision 的 fallback; K3 探针实测 image_url → HTTP 400 InvalidParameter,
  当初 under-declare 的安全方向被证明是对的 (V37.9.339 同款判断)。
...
class KimiK3Provider(BaseProvider):
    ...
    capabilities = ProviderCapabilities(
        ...
        json_mode=True,        # V37.9.347 实测翻案: response_format=json_object 返回纯 JSON
        ...
        tier_evidence="V37.9.346 Mac Mini E2E 3/3: text(stop + content '$0.05') / tool_calling(finish_reason=tool_calls + get_weather Tokyo) / streaming(478 SSE 行); reasoning 字段 + reasoning_tokens=190 证 reasoning 通道 (V37.9.345 保守 False 经实测翻案). V37.9.347 补齐: json_mode 实测支持 (纯 JSON 可解析) 翻案为 True / vision 实测 HTTP 400 InvalidParameter 坐实 False",
    )
