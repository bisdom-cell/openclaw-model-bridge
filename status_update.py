#!/usr/bin/env python3
"""
status_update.py — 三方共享意识锚点（原子读写 + 跨写者 RMW 锁）
两层并发安全（审计 finding C 全闭环）：
  1. V37.9.237 唯一 tmp（pid 后缀）+ os.replace 原子发布——并发写者绝不共用固定
     tmp 路径交错损坏（损坏 → 读者 fallback DEFAULT_STATUS = 系统失忆，已消除）。
  2. V37.9.238 status_lock() 跨写者 RMW 锁（fcntl.flock 独立锁文件 + 有界获取 +
     FAIL-OPEN）——防两个并发 load→mutate→save 互相覆盖（lost-update）。CLI main()
     与全部库 RMW 调用方（daily_observer/preference_learner/security_score）已包裹。
     FAIL-OPEN 语义：锁超时/不可用时无锁继续（最坏 last-writer-wins），锁是增强非闸门。
...
def status_lock(timeout=5.0, poll=0.1):
    """跨写者 RMW 锁（V37.9.238，审计 finding C lost-update 半边）。

    包住整个 load_status() → mutate → save_status() 循环，防两个并发 RMW 写者
    互相覆盖（如 recent_changes insert(0) 静默消失）。设计三原则（防死锁）:
      1. **fcntl.flock 独立锁文件**（STATUS_FILE + ".lock"）——不能 flock 目标文件
         本身（os.replace 换 inode 使锁失效）；flock 由内核管理，进程死亡（含
         SIGKILL）自动释放，无 mkdir 目录锁的 stale-lock 永久阻塞问题。
      2. **有界获取**：LOCK_NB 轮询 ≤ timeout 秒，绝不无限阻塞 cron 写者。
      3. **FAIL-OPEN**：超时 / fcntl 不可用 / 锁文件不可创建 → stderr WARN 后
         无锁继续（= 今日行为，最坏 last-writer-wins），锁永远只是增强不是闸门。

    用法: ``with status_lock(): data = load_status(); ...; save_status(data)``。
    save_status 本身**不**内嵌锁（同进程二次 flock 会自阻塞→假超时），RMW 调用方
    显式包裹。yield 布尔值 = 是否真持锁（诊断用，调用方无需检查）。
    """
            ...
            while True:
                try:
                    ...
                    if time.time() >= deadline:
                        print("[status_update] WARN: status.lock 获取超时(%.1fs)，"
                              "无锁继续(fail-open)" % timeout, file=sys.stderr)
                        break
                    time.sleep(poll)
        except (ImportError, OSError) as e:
            print("[status_update] WARN: status lock 不可用(%s)，无锁继续(fail-open)" % e,
                  file=sys.stderr)
        yield acquired
    finally:
        if fd is not None:
...
def main():
    ...
    # V37.9.238（finding C lost-update 半边）：整个 RMW 在跨写者锁内执行。
    # --read 路径不锁（atomic publish 保证读一致，锁只保护 read-modify-write）。
    with status_lock():
        _run_rmw(args, parser)


def _run_rmw(args, parser):
    """CLI 的 read-modify-write 主体（V37.9.238 从 main() 提取，锁内执行）。"""
    data = load_status()
    changed = False
