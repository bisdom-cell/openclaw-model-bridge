...
class TestVerifyProvidersToAdapterIntegration(unittest.TestCase):
    ...
    def test_full_visibility_no_drift(self):
        from unittest.mock import patch, MagicMock
        # V37.9.204: doubao + deepseek + deepseek_full → declared 集合 10 个 (7 built-in + 3 真插件)
        all_known = {"qwen", "openai", "gemini", "claude", "kimi", "minimax", "glm", "doubao", "deepseek", "deepseek_full"}
        body = json.dumps({
            "provider": "qwen",
            "fallback": "gemini",
...
class TestV37958DryRunActivation(unittest.TestCase):
    ...
    def test_audit_metadata_version_v3_36_or_later(self):
        ...
        valid_versions = (
            ...
            'version: "3.56"',  # V37.9.129 +1 INV-FALLBACK-EXCLUDE-001 (gemini geo-block 退役)
        )
        self.assertTrue(
