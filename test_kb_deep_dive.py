...
class TestOaResolutionV183(unittest.TestCase):
    ...
    # --- V37.9.262 OpenAlex 第二 OA 源（mock _openalex_fetch）---
    def test_openalex_best_oa_location_pdf(self):
        data = {"best_oa_location": {"pdf_url": "https://repo.org/p.pdf"}}
        with patch.object(m, "_openalex_fetch", return_value=data):
            self.assertEqual(m._openalex_lookup_oa_pdf("10.1/2"),
                             "https://repo.org/p.pdf")

    def test_openalex_primary_location_fallback(self):
        # best_oa_location 无 pdf → primary_location.pdf_url
        data = {"best_oa_location": {"pdf_url": None},
                "primary_location": {"pdf_url": "https://pmc.org/p.pdf"}}
        with patch.object(m, "_openalex_fetch", return_value=data):
            self.assertEqual(m._openalex_lookup_oa_pdf("10.1/2"),
                             "https://pmc.org/p.pdf")

    def test_openalex_open_access_oa_url_fallback(self):
        data = {"open_access": {"oa_url": "https://oa.org/paper.pdf"}}
        with patch.object(m, "_openalex_fetch", return_value=data):
            self.assertEqual(m._openalex_lookup_oa_pdf("10.1/2"),
                             "https://oa.org/paper.pdf")

    def test_openalex_arxiv_abs_normalized(self):
        data = {"best_oa_location": {"pdf_url": "https://arxiv.org/abs/2501.5"}}
        with patch.object(m, "_openalex_fetch", return_value=data):
            self.assertEqual(m._openalex_lookup_oa_pdf("10.1/2"),
                             "https://arxiv.org/pdf/2501.5.pdf")

    def test_openalex_closed_paper_returns_none(self):
        # 真闭源: is_oa=false, best_oa_location=null → None（维持 abstract_only）
        data = {"best_oa_location": None, "primary_location": {"pdf_url": None},
                "open_access": {"is_oa": False, "oa_url": None}}
        with patch.object(m, "_openalex_fetch", return_value=data):
            self.assertIsNone(m._openalex_lookup_oa_pdf("10.1/2"))

    def test_openalex_fetch_failed_fail_open(self):
        with patch.object(m, "_openalex_fetch", return_value=None):
            self.assertIsNone(m._openalex_lookup_oa_pdf("10.1/2"))

    def test_openalex_empty_doi(self):
        self.assertIsNone(m._openalex_lookup_oa_pdf(""))

    def test_openalex_fetch_network_error_fail_open(self):
        with patch("kb_deep_dive.urllib.request.urlopen",
                   side_effect=m.urllib.error.URLError("boom")):
            self.assertIsNone(m._openalex_fetch("10.1/2"))

    # --- V37.9.262 resolve_oa_pdf_url DOI 链式: S2 → OpenAlex ---
    def test_resolve_doi_s2_hit_shortcircuits_openalex(self):
        # S2 命中 → 不调 OpenAlex（短路）
        with patch.object(m, "_s2_lookup_oa_pdf", return_value="https://s2.org/p.pdf"), \
             patch.object(m, "_openalex_lookup_oa_pdf") as mk_oa:
            r = m.resolve_oa_pdf_url("https://doi.org/10.1145/3774904.3792985")
        self.assertEqual(r, "https://s2.org/p.pdf")
        mk_oa.assert_not_called()

    def test_resolve_doi_s2_miss_falls_through_to_openalex(self):
        # 血案场景: S2 无 OA（no PDF URL derivable）→ OpenAlex 捞回合法 OA
        with patch.object(m, "_s2_lookup_oa_pdf", return_value=None), \
             patch.object(m, "_openalex_lookup_oa_pdf",
                          return_value="https://oa.org/recovered.pdf") as mk_oa:
            r = m.resolve_oa_pdf_url("https://doi.org/10.1145/3774904.3792985")
        self.assertEqual(r, "https://oa.org/recovered.pdf")
        mk_oa.assert_called_once_with("10.1145/3774904.3792985")

    def test_resolve_doi_both_miss_returns_none(self):
        # 真闭源: S2 与 OpenAlex 都无 → None（诚实 abstract_only）
        with patch.object(m, "_s2_lookup_oa_pdf", return_value=None), \
             patch.object(m, "_openalex_lookup_oa_pdf", return_value=None):
            self.assertIsNone(
                m.resolve_oa_pdf_url("https://doi.org/10.1016/J.RESS.2026.1"))
